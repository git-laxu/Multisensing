# backend/common_sensor/base_data_manager.py
# -*- coding: utf-8 -*-
...
class BaseSensorDataManager:
    def __init__(self, base_save_dir: Path, sensor_type: str):
    ...
    def list_sessions(self, project_name: str) -> List[Dict[str, Any]]:
        ...
        for session_dir in session_dirs:
            ...
            results.append({
                ...
                "total_size_mb": round(total_size_bytes / (1024 * 1024), 2),
                "artifacts": {
                    "csv_file_count": self._count_files(data_dir, "*.csv"),
                    "jsonl_file_count": self._count_files(logs_dir, "*.jsonl"),
                    "has_csv": self._count_files(data_dir, "*.csv") > 0,
                    "has_jsonl": self._count_files(logs_dir, "*.jsonl") > 0,
                }
            })
    ...
    def delete_session(self, project_name: str, session_id: str) -> Dict[str, Any]:
        ...
        return {
            "success": True,
            "message": f"{self.sensor_type} session 删除成功",
    ...
    def delete_project(self, project_name: str) -> Dict[str, Any]:
        ...
        return {
            "success": True,
            "message": f"{self.sensor_type} 项目删除成功",
            "project_name": self._normalize_project_name(project_name),
            "sensor_type": self.sensor_type,
        }
